...
void WorldSession::handleLogoutRequestOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    // Логика как в Trinity/sgl-panda-core для MoP: всегда сначала SMSG_LOGOUT_RESPONSE
...
void WorldSession::sendAddonInfo()
{
    ...
    for (auto itr : m_addonList)
    {
        ...
        {
            // Trinity/Panda: write public key bytes in pubKeyOrder order (output[i] = PublicKey[publicKeyOrder[i]])
            for (int i = 0; i < 256; i++)
