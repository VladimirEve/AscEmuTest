...
void TileMap::load(char* filename)
{
    ...
    const bool trinityMapFormat = (header.versionMagic == MAP_VERSION_TRINITY);
    ...
    if (header.liquidMapOffset != 0)
        loadLiquidData(f, header, trinityMapFormat);
...
void TileMap::loadLiquidData(FILE* f, TileMapHeader& header, bool trinityLiquidFormat)
{
